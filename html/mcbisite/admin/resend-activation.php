...
if(!empty($_POST) && $emailActivation)
{
		...
		if(count($errors) == 0)
		{
			...
			if(!emailloginLinked($email,$login))
			{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						if(!updatelast_activation_request($new_activation_token,$login,$email))
						{
						...
						else
						{
							...
							$hooks = array(
								"searchStrs" => array("#ACTIVATION-URL","#USERNAME#", "#WEBLOGO", "#WEBSITEURL#", "#NETWORK"),
								"subjectStrs" => array($activation_url,$userdetails["login"], $websiteLogo, $websiteUrl,"")
							);
							...
							{
								if(!$mail->sendMail($userdetails["email"],"Link de activación"))
								{
