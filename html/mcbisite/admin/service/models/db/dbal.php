...
class dbal
{
	...
	*/
	...
	/**
	* Fetch all rows array format
	*/
	function sql_fetchrowset_array($query_id = false)
	{
		if ($query_id === false)
		{
			$query_id = $this->query_result;
		...
		{
			$result = array();
			while ($row = $this->sql_fetchrow_array($query_id))
			{
				$result[] = $row;
			}
	...
	/**
	* Fetch last row inserted
	*/
	function sql_lastinsertion_array($query_id = false)
	{
		error_log("sql_lastinsertion_array");
		return ($this->db_connect_id) ? @mssql_fetch_array($query_id) : false;
	}
